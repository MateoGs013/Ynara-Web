...
/* ─────────────────────────────────────────────
   Inline styles — self-contained, no globals.css pollution.
   Using a <style> tag in JSX (styled-jsx pattern, same as
   how existing components handle page-local CSS).
───────────────────────────────────────────── */
const pageStyles = `
  /* ── Spacer / intro pin target ─────────────────── */
  .hl-intro {
    position: relative;
    min-height: 100svh;
    display: flex;
    ...
    /* transparent → WebGL field shows through */
  ...
  /* ── Text-pin block (secondary pin target) ──────── */
  .hl-text-pin {
    padding: 6vw 6vw 4vw;
    background: transparent;
  }
  ...
  .hl-exit__body {
    font-family: var(--font-body), "DM Sans", system-ui, sans-serif;
    font-size: clamp(1rem, 0.6rem + 1.2vw, 1.25rem);
    line-height: 1.6;
    color: var(--c-navy);
    opacity: 0.7;
    max-width: 560px;
